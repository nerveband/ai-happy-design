...
namespace ai
{
	namespace detail
	{
		class AIErrorThrowerBase
		{
		public:
			AIErrorThrowerBase(AIErr err = kNoErr, const char* whatMessage = nullptr) : fError(err), mWhatMessage(whatMessage)
			{
				ai::check_ai_error(fError, mWhatMessage);
			}

			AIErrorThrowerBase& operator=(AIErr err)
			{
				fError = err;
				ai::check_ai_error(fError, mWhatMessage);
				return (*this);
...
#define ERROR_DETAIL "AI exception from ErrorThrower declared in file: " __FILE__ " line: " LINE_STRING " "
#define ERROR_CLASS_NAME AI_ANONYMOUS_VARIABLE(AIErrorThrower_)

#define AIErrorThrower \
struct ERROR_CLASS_NAME : public ai::detail::AIErrorThrowerBase \
{ \
	ERROR_CLASS_NAME(AIErr err = kNoErr) : ai::detail::AIErrorThrowerBase(err, ERROR_DETAIL){} \
	ERROR_CLASS_NAME& operator=(AIErr err) { ai::detail::AIErrorThrowerBase::operator=(err); return *this; } \
}
