...
#define kAIFileFormatSuiteVersion16		AIAPI_VERSION(16)			// From AI 27.7
#define kAIFileFormatSuiteVersion		kAIFileFormatSuiteVersion16
...
/** @ingroup Selectors
    Sent to file format plug-ins to execute the preloading of files in parallel tasks.
    \li A Preload operation Reads the file and keeps the data in a cache.
 	\li This will help reducing the file opening time when for example an AI file has large number of links.
 	\li A file format plugin can register for proloading by sending kFileFormatSupportsPreload in extendedOptions.

    The data is an \c #AIFileFormatMessage.
     */
#define kSelectorPreloadFileFormat			"AI Preload"

/** @ingroup Selectors
	Sent to file format plug-ins to clear a specific file's data from the preload cache.
	\li Signals the file format plugin to clear the cached data read during preload.

	The data is an \c #AIFileFormatMessage.
	 */
#define kSelectorAIRemoveFromFileFormatCache	"AI Remove From Cache"

/** @ingroup Selectors
    Sent to file format plug-ins to Clear the entire cache of preloaded files.
 	\li Signals the file format plugin to clear the cached data read during preload.

    The data is an \c #AIFileFormatMessage.
     */
#define kSelectorAIClearFileFormatCache		"AI Clear Cache"

#define kExportDoneNotifier 		"AI Export Completed Notifier"
...
/** @ingroup Errors
    Plug-in file format could not complete the operation either
    because the artboards contains no arts or document has no arts.*/
#define    kEmptySelectionError                        '!SEL'
...
enum AIFFExtendedOptions{
    ...
    /*This option specifies that a file format supports preloading linked file data.
     	to optimize the file openning time*/
    kFileFormatSupportsPreload = (1 << 5),
    /* This option specifies that a file format supports unembedding embedded images.*/
    kFileFormatUnEmbed = (1 << 6),
    /* This option specifies that a preloaded file format is blocking or not.
     If this flag is set then we dont wait for file if its not loaded but return kPreloadNotReadyErr. */
    kFileFormatPreloadIsNonBlocking = (1 << 7)
};
...
struct AIFileFormatMessage {
	...
	/** Document name-space to be used in shared-memory */
	ai::UnicodeString documentNameSpace;
...
enum AIDocumentStartupProfile
{
	...
	kAIStartupProfileSocial,
	kAIStartupProfileBranding,
...
\endverbatim
	...
	*/
	...
	/** Invokes the File > Save dialog, allowing you to filter the available file formats.
	  ...
	  */
	AIAPI AIErr (*PutFile) (ai::int32 options, const ai::UnicodeString& dlgTitle, ai::FilePath &fileSpec, AIFileFormatHandle *fileFormat, AIBoolean *good );
