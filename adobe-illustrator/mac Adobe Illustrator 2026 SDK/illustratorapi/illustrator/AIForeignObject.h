...
#define kAIForeignObjectSuiteVersion6	AIAPI_VERSION(6)
/** Foreign Object Suite version */
#define kAIForeignObjectSuiteVersion	kAIForeignObjectSuiteVersion6
/** Foreign Object Suite version */
...
typedef struct AIForeignObjectSuite {
	...
	/** Sets the raster info of the foreign object (used for linked images)
			@param art The foreign object
			@param info The raster info.
            @param transferReference When true, foreign object does not add a ref count to the raster passed via info, otherwise it does. The caller must not dispose the raster in the former case, but do so in the latter case.
		*/
	AIAPI AIErr (*SetRasterInfo) (AIArtHandle art, const FORasterInfo& info, bool transferReference);
	
	/** Retrieves the raster info of the foreign object (used for linked images)
			@param art The foreign object
			@param info The raster info.
		*/
	AIAPI AIErr (*GetRasterInfo) (AIArtHandle art, FORasterInfo& info);
