...
enum AIHitRequest {
	...
	kStrokeHitRequest,
	/** Report hits on paintable objects (non-guide paths & text) and hits on (focal) strokes of paths. The hit tolerance is interpreted
			as being a distance from the stroked area instead of a distance from the
					bezier. The AIHitType will be returned as either kSegmentHitType or kPHitType,
							depending on the nearest point on the path. */
	kPaintStrokeHitRequest,
	/** Report hits on paintable objects (non-guide paths & text) , hits on (focal) strokes of paths and hits on clipping path.
		The hit tolerance is interpreted as being a distance from the stroked area instead of a distance from the
		bezier. The AIHitType will be returned as either kSegmentHitType or kPHitType,
		depending on the nearest point on the path. */
	kPaintStrokeWithClipHitRequest
};
