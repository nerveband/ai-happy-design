...
#define kAILiveEffectSuiteVersion			AIAPI_VERSION(8)
...
/** Bit flags for Live Parallel Effect Execution Art properties
...
enum class AIParallelExecutionArtFlags : ai::int32 {
	/** Supports running the effect on all art types and is the default value. */
	kNoArtFlags 			= 0,
	/** Parameter is used to if plgin support outline text for a text art */
	kOutlineTextArt			= 0x1,
	/* Set flag if parallel effect execution enabled on text during live edit */
	kLiveEditTextSupportForParallelExecution = 0x2,
	/** Flag to indicate if global plugin objects are required to be copied into snippet doc  */
	kCopyGlobalObjectsForParallelExecution = 0x4,
	/** Flag to indicate parallel execution required special check for path on art plugin  */
	kCheckBrushParamForParallelExecution = 0x8
};
...
struct AILiveEffectData {
	...
	ai::int32 majorVersion = 0;
	/** Minor version number of this plug-in. */
	ai::int32 minorVersion = 0;
	/** The type of art this effect can operation on,
		a \c #AIStyleFilterPreferredInputArtType value.*/
	ai::int32 prefersAsInput = 0;
	/** The type and supported features of this effect,
		a logical OR of \c #AIStyleFilterFlags. */
	ai::int32 styleFilterFlags = 0;
};

/** Information supplied when registering a Live Effect for Parallel Execution
	with \c #AILiveEffectSuite::AddLiveEffect(). */
struct AILiveEffectParallelExecutionData {
	/** The supported art for this effect for parallel execution,
		a logical OR of \c #AIParallelExecutionArtFlags. */
	ai::int32 parallelExecutionArtFlags = 0;
};
...
struct AILiveEffectSuite {
	...
	*/
	...
	/** Add data for Parallel Execution.
			@param liveEffectHandle The effect object.
			@param parallelExecutionData Data to customize parallel execution behavior.
		*/
	AIAPI AIErr (*SetLiveEffectParallelExecutionData) ( AILiveEffectHandle liveEffectHandle , const AILiveEffectParallelExecutionData& parallelExecutionData );
};
