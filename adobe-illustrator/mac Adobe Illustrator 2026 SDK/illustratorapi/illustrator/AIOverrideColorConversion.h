...
#define kAIOverrideColorConversionSuiteVersion9	AIAPI_VERSION(9)
#define kAIOverrideColorConversionSuiteVersion	kAIOverrideColorConversionSuiteVersion9
...
typedef struct {
	...
	*/
    ...
    /**
     Direct conversion of color with AGM
     Can be used with multiple parallel threads
        @param srcCS Color space for source buffer, an \c #AIWorkingColorSpace.
        @param srcFloat A buffer containing source color
        @param dstCS Color space for destination buffer, an \c #AIWorkingColorSpace.
        @param dstFloat [out] A buffer which returns converted color
        @param hasAlpha aplha value for source and destination
     ...
     */
    AIAPI AIErr ( *ConvertColorWithAGMSpace)(AIWorkingColorSpace srcCS, AIFloat* srcFloat, AIWorkingColorSpace dstCS, AIFloat* dstFloat, bool hasAlpha, const AIColorConvertOptions& options);
