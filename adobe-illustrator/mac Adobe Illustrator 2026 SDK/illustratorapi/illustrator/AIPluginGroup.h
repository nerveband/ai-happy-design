...
enum AIPluginGroupOptions {
	...
	*/
	...
	/** The flag indicates enabling support for parallel execution for plugin groups.
		This Live Effect plugin is capable of being invoked on non-main threads concurrently
		to process multiple art objects within different or the same documents. The plugin is
		responsible for ensuring thread safety by avoiding data races, deadlocks, and other
		concurrency issues when this flag is enabled. */
	 kPluginGroupEnableForParallelExecution	= (1<<22),
};
