...
enum AIRasterFlags {
	...
	/** Used to specify either we need to create this image in shared space or
	 we need to use the heap memory space to create storage for this image*/
	kRasterCreatedInSharedSpace	= 0x0010,
	/** Used to specify if we need to create this image in single buffer in
	 the heap memory space*/
	kRasterCreateInSingleBuffer	= 0x0020,
	/** Used to specify whether the image is a linked image*/
	kRasterLinkedImage		= 0x0040,
	/** Used to specify whether the image is a non-composite image.
	Non-composite images are layered images which can be imported as ai objects */
	kRasterNonComposite = 0x0080,
	/** Used to specify whether the image is a gradient dither image.
	Gradient dither images use nearest neighbor sampling instead of average downsampling */
	kRasterGradientDither = 0x0100
};
...
GWorldTile.channelInterleave[3] = 2; // Map 32-bit blue value to 24-bit blue value
	...
	This example sets the art-object slices \c front and \c back values
...
artSlice.back = 3;
	...
	GWorld back to the raster art object, the GWorlds slice controls the
...
typedef struct AIRasterSuite {
	...
	/** Sets the transformation matrix for a raster art object.
		All raster data is internally kept at 72 PPI and uses the objects
	...
	/** Copies pixel data in a local buffer (a tile) to all or part of
		a raster art objects pixel map. This can be used, for instance,
