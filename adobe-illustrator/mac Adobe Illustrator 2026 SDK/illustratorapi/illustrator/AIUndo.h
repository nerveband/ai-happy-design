...
#define kAIUndoSuiteVersion		AIAPI_VERSION(12)
#define kAIUndoVersion			kAIUndoSuiteVersion

/** @ingroup Notifiers
	 Sent when a new undoable transaction is added. */
#define kAIUndoNewTransaction				"AI Undo New Transaction Notifier"

/** @ingroup Notifiers
	 Sent when the last undoable transaction in the future queue is discarded. */
#define kAIUndoForgetFutureTransaction		"AI Undo Forget Last Future Transaction"

/** @ingroup Notifiers
	Sent after the future queue is discarded. */
#define kAIUndoClearedAllFutureTransaction	"AI Undo Cleared All Future Transaction"

/** @ingroup Notifiers
	 Sent when the first undoable transaction in the past queue is commited. */
#define kAIUndoCommitPastTransaction		"AI Undo Commit First Past Transaction"

/** @ingroup Notifiers
     Sent when the undo stack is swapped with a new one */
#define kAIUndoStackRefresh					"AI Undo Stack Refresh"
...
struct AIUndoNotificationParameters
{
	ai::int32			fNoOfTransactions { 0 } ;
	AIDocumentHandle	fDocument { nullptr };
	
	AIUndoNotificationParameters(ai::int32 _fNoOfTransactions, AIDocumentHandle _fDocument)
		: fNoOfTransactions { _fNoOfTransactions }
		, fDocument { _fDocument }
	{}
};
...
typedef struct {
	...
	*/
	...
	/** Specifies the localizable text(Unicode String) for Undo, Redo, and History Panel  menu items.
	...
	*/
	AIAPI AIErr (*SetUndoRedoCmdTextUS) ( const ai::UnicodeString& undoText, const ai::UnicodeString& redoText,  const ai::UnicodeString& cmdText);

	/** Undo "N" changes from the past.
	@param n	The no. of transactions to be undone.
	 */
	AIAPI AIErr (*MultiUndoTransaction) ( AIDocumentHandle document, ai::int32 n );

	/** Redo "N" changes from the future.
	@param n	The no. of transactions to be redone.
	 */
	AIAPI AIErr (*MultiRedoTransaction) ( AIDocumentHandle document, ai::int32 n );
