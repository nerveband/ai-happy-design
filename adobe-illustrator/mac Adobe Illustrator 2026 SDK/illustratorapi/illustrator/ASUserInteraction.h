...
#ifdef AUTO_SUITE_INTERACTION_LEVEL
#include "AutoSuite.h"
use_suite_optional_1(ASUserInteraction)

class AutoInteractionLevelSetReset
{
public:
	AutoInteractionLevelSetReset(ASInteractionAllowed interactionLevel)
	{
		if (s1ASUserInteraction)
		{
			mInteractionAllowed = s1ASUserInteraction->GetInteractionAllowed();
			mError = s1ASUserInteraction->SetInteractionAllowed(interactionLevel);
		}
	}
	~AutoInteractionLevelSetReset()
	{
		if (s1ASUserInteraction && mError == kNoErr)
			mError = s1ASUserInteraction->SetInteractionAllowed(mInteractionAllowed);
	}
private:
	ASInteractionAllowed mInteractionAllowed{};
	ASErr mError = kBadParameterErr;
};
#endif //AUTO_SUITE_INTERACTION_LEVEL
