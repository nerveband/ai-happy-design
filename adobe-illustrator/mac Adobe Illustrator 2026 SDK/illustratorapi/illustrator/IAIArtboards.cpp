...
namespace ai
{
	...
	AIErr ArtboardProperties::GetUUID(ArtboardUUID& uuid) const
	{
		return sAIArtboard->GetUUID(*this, uuid);
	}

	AIErr ArtboardProperties::GetLocked(AIBoolean& isLocked) const
	{
		return sAIArtboard->GetLocked(*this, isLocked);
	}

	AIErr ArtboardProperties::SetLocked(const AIBoolean& isLocked)
	{

		return sAIArtboard->SetLocked(*this, isLocked);
	}

	AIErr ArtboardProperties::GetHide(AIBoolean& isHidden) const
	{
		return sAIArtboard->GetHide(*this, isHidden);
	}

	AIErr ArtboardProperties::SetHide(const AIBoolean& isHidden)
	{
		return sAIArtboard->SetHide(*this, isHidden);
	}
	...
	AIErr ArtboardProperties::SetColor(const AIColor &color)
	{
		return sAIArtboard->SetColor(*this, color);
	}

	AIErr ArtboardProperties::GetColor(AIColor &color) const
	{
		return sAIArtboard->GetColor(*this, color);
	}
	...
	namespace ArtboardUtils
	{
		...
		*/
		...
		AIErr GetNumSelectedArtboards(ai::ArtboardID& count)
		{
			...
			try
			{
				ai::ArtboardID selectedCount = 0;
				ai::ArtboardList artboardList;
				for (const auto& artboard: artboardList)
				{
					const auto artboardProps = artboard.GetProps();
					AIBoolean isArtboardSelected = false;
					error = artboardProps.IsSelected(isArtboardSelected);
