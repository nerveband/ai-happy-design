...
namespace
{
	...
	const ai::LiteralString& GetAIMode(const ai::IDataFilter::Mode mode) AINOEXCEPT
	{
		switch (mode)
		{
			case ai::IDataFilter::Mode::kRead: return kModeReadStr;
			case ai::IDataFilter::Mode::kWrite: return kModeWriteStr;
			case ai::IDataFilter::Mode::kAppend: return kModeAppendStr;
			default:
...
namespace ai
{
	...
	public:
		...
		Mode GetMode() const AINOEXCEPT { return mMode; }

		bool CanRead() const AINOEXCEPT override;
		void Read(char* store, size_t& count) override;
		bool CanWrite() const AINOEXCEPT override;
	...
	public:
		...
		{
			const auto& aiMode = GetAIMode(GetMode());
	...
	public:
		using inherited = BaseDataFilter;
		explicit ZStdDataFilter(IDataFilter::Mode filterMode, ai::int32 compressionLevel)
			:inherited(filterMode)
		{
			const auto& aiMode = GetAIMode(GetMode());
			mError = sAIDataFilter->NewZStdDataFilter(aiMode, compressionLevel, &mFilter);
		}
	};
	...
	public:
		...
		bool CanRead() const AINOEXCEPT override { return true; }
		bool CanWrite() const AINOEXCEPT override { return true; };
	};
	...
	class GrowableBufferDataFilter : public BaseDataFilter
	{
	public:
		using inherited = BaseDataFilter;
		explicit GrowableBufferDataFilter(uint8 baseExp, uint8 incExp)
		{
			mError = sAIDataFilter->NewGrowableBufferDataFilter(baseExp, incExp, &mFilter);
			mError = sAIDataFilter->LinkDataFilter(nullptr, mFilter);
		}
		bool CanRead() const AINOEXCEPT override { return true; }
		bool CanWrite() const AINOEXCEPT override { return true; };
	...
	bool BaseDataFilter::CanRead() const AINOEXCEPT
	{
		return mMode == Mode::kRead;
	}
	...
	bool BaseDataFilter::CanWrite() const AINOEXCEPT
	{
		return mMode == Mode::kWrite || mMode == Mode::kAppend;
	}
...
#ifdef AI_USE_BUFFER_WITH_FILE_DATAFILTER
		// Creating BufferDataFilter before FileDataFilter for Web environment as writing to
		// persistent file system is slow in emscripten when written in smaller chunks
		constexpr size_t kBufferSizeForFileDataFilter = 1 * 1024 * 1024;
		if(filter && !nextFilter)
		{
			nextFilter = ai::CreateBufferDataFilter(kBufferSizeForFileDataFilter);
		}
#endif
	...
	std::unique_ptr<ai::IDataFilter> CreateZStdDataFilter(
		IDataFilter::Mode filterMode,
		ai::int32 compressionLevel,
		std::unique_ptr<IDataFilter> nextFilter)
	{
		std::unique_ptr<ai::IDataFilter> filter(std::make_unique<ZStdDataFilter>(filterMode, compressionLevel));
		if (filter && nextFilter)
		{
			filter->Link(std::move(nextFilter));
		}
	...
	std::unique_ptr<IDataFilter> CreateGrowableBufferDataFilter(
		uint8 baseExp /*= DataFilter::kAIGrowableBufferDefaultBaseExp*/,
		uint8 incExp /*= DataFilter::kAIGrowableBufferDefaultIncExp*/,
		std::unique_ptr<IDataFilter> nextFilter /*= nullptr*/)
	{
		std::unique_ptr<ai::IDataFilter> filter(std::make_unique<GrowableBufferDataFilter>(baseExp, incExp));
		if (filter && nextFilter)
		{
			filter->Link(std::move(nextFilter));
		}
