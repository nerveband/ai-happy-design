...
*/
	...
	public:
		...
		*/
		...
		/** 
			Returns true if filter can be read from
		*/
		virtual bool CanRead() const AINOEXCEPT = 0;
		...
		*/
		...
		/** 
			Returns true if filter can be written to
		*/
		virtual bool CanWrite() const AINOEXCEPT = 0;
	...
	*/
	std::unique_ptr<IDataFilter> CreateZStdDataFilter(
		IDataFilter::Mode filterMode,
		ai::int32 compressionLevel = ai::DataFilter::kAIZstdDefaultCompressionLevel,
		std::unique_ptr<IDataFilter> nextFilter = nullptr
	);
...
*/
	...
	*/
	...
	/** Create a stack-based Growable Buffer Data Filter.
		#AIDataFitlerSuite::NewGrowableBufferDataFilter()

		// Creates growable buffer with initial size of 16 MB (2^24),
		// with each subsequent buffer doubling in size till 64 MB (2^(24+2)).
		// Thereafter the subsequent buffers are created of 64 MB.
		auto filter = CreateGrowableBufferDataFilter(24, 2);	
