...
namespace ai
{
	...
	AIEntryRef Entry::release() const
	{
	...
	public:
		...
		void move_entry(ai::Array& srcArr, ai::int32 srcArrIndex, ai::int32 dstArrIndex)
		{
			mError = sAIArray->MoveEntry(srcArr.pImpl->mArray, mArray, srcArrIndex, dstArrIndex, nullptr);
		}

		void swap_entries(ai::Array& otherArr, ai::int32 otherArrIndex, ai::int32 thisArrIndex)
		{
			mError = sAIArray->SwapEntries(mArray, otherArr.pImpl->mArray, thisArrIndex, otherArrIndex);
		}
	...
	void Array::move_entry(ai::Array& srcArr, ai::int32 srcArrIndex, ai::int32 dstArrIndex)
	{
		pImpl->move_entry(srcArr, srcArrIndex, dstArrIndex);
	}

	void Array::swap_entries(ai::Array& otherArr, ai::int32 otherArrIndex, ai::int32 thisArrIndex)
	{
		pImpl->swap_entries(otherArr, otherArrIndex, thisArrIndex);
	}
	...
	public:
		...
		AIErr GetArtEntry(KeyType key, AIArtHandle& art)
		{
			return sAIDictionary->GetArtEntry(mDict, key, &art);
		}

		AIErr NewArtEntry(KeyType key, ai::int16 type)
		{
			return sAIDictionary->NewArtEntry(mDict, key, type);
		}

		AIErr MoveArtToEntry(KeyType key, AIArtHandle art)
		{
			return sAIDictionary->MoveArtToEntry(mDict, key, art);
		}

		AIErr MoveEntryToArt(KeyType key, ai::int16 paintOrder, AIArtHandle prep, AIArtHandle& outArt)
		{
			return sAIDictionary->MoveEntryToArt(mDict, key, paintOrder, prep, &outArt);
		}

		AIErr CopyArtToEntry(KeyType key, AIArtHandle art)
		{
			return sAIDictionary->CopyArtToEntry(mDict, key, art);
		}

		AIErr CopyEntryToArt(KeyType key, ai::int16 paintOrder, AIArtHandle prep, AIArtHandle& outArt)
		{
			return sAIDictionary->CopyEntryToArt(mDict, key, paintOrder, prep, &outArt);
		}
	...
	AIErr Dictionary::GetArtEntry(KeyType key, AIArtHandle& art)
	{
		return pImpl->GetArtEntry(key, art);
	}

	AIErr Dictionary::NewArtEntry(KeyType key, ai::int16 type)
	{
		return pImpl->NewArtEntry(key, type);
	}

	AIErr Dictionary::MoveArtToEntry(KeyType key, AIArtHandle art)
	{
		return pImpl->MoveArtToEntry(key, art);
	}	

	AIErr Dictionary::MoveEntryToArt(KeyType key, ai::int16 paintOrder, AIArtHandle prep, AIArtHandle& outArt)
	{
		return pImpl->MoveEntryToArt(key, paintOrder, prep, outArt);
	}

	AIErr Dictionary::CopyArtToEntry(KeyType key, AIArtHandle art)
	{
		return pImpl->CopyArtToEntry(key, art);
	}

	AIErr Dictionary::CopyEntryToArt(KeyType key, ai::int16 paintOrder, AIArtHandle prep, AIArtHandle& outArt)
	{
		return pImpl->CopyEntryToArt(key, paintOrder, prep, outArt);
	}
    ...
    Dictionary Dictionary::GetDocumentNonRecordedDictionary(AIDocumentHandle docHandle)
    {
        ai::Expects(docHandle);
        ai::Ref<AIDictionaryRef> nonRecordedDictRef;
        ai::check_ai_error(sAIDocument->GetNonRecordedDictionaryForDocument(docHandle, nonRecordedDictRef << ai::Replace));
        return ai::Dictionary(nonRecordedDictRef);
    }
