...
public:
	...
	public:
		...
		using value_type = UnicodeString::value_type;
		using difference_type = ptrdiff_t;
		using pointer = UnicodeString::value_type*;
		using reference = UnicodeString::value_type&;
...
#include<string_view>
template <>
struct std::hash<ai::UnicodeString>
{
	std::size_t operator()(const ai::UnicodeString& key) const
	{
		const ai::UnicodeString::UTF16Char* buffer = nullptr;
		auto len = key.utf_16(buffer);
		return std::hash<std::u16string_view>{}(std::u16string_view((char16_t*)buffer, len));
	}
};
