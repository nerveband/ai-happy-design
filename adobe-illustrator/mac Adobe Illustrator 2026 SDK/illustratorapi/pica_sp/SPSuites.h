...
struct SuiteNameVersion
{
	std::string mName;
	ai::int32 mAPIVersion{ 0 };
	std::string mPluginName;
	SuiteNameVersion(std::string name = {}, ai::int32 apiVersion = 0, std::string pluginName = {})
		:mName{ name }, mAPIVersion{ apiVersion }, mPluginName{ pluginName }
	{

	}

	bool operator < (const SuiteNameVersion& other) const
	{
		if (mName < other.mName)
			return true;
		else if (mName == other.mName)
			return mAPIVersion < other.mAPIVersion;
...
SPAPI std::set<SuiteNameVersion> SPGetAcquireFailedSuites();
#endif
