...
def make_4_byte_hex_string(number_string):
	...
	return hexed_string.decode('utf-8')
...
class pipl:
	def __init__(self):
		...
		fileO = open(dir_path + '/template_plugin.json', 'r', encoding= "utf-8")
...
##############################################
#Add Plugin adpt property
##############################################
	def add_plugin_adpt(self, value, id = 0):
		dict1 = {}
		dict1['id'] = id
		dict1['vendor'] = 'ADBE'
		dict1['value' ]=  value
		self.data['adpt']= dict1
...
def generate_pipl_bin_recursively_from_dict(pipl_dict, file_obj):
	...
	for key in pipl_dict:
		if key == 'win':
			...
			'''if(count == 2):'''
			file_obj.write(str(out_prop_value))
