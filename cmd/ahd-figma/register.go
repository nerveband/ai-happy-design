...
var registerCmd = &cobra.Command{
	...
	Long: `Auto-detects installed AI editors and registers ahd-figma as an MCP server.
...
func registerClaudeCode(binaryPath string) error {
	...
	cmd := exec.Command("claude", "mcp", "add-json", "ahd-figma", "--scope", "user")
...
func registerJSONConfig(home string, editor editorConfig, binaryPath string) error {
	...
	if _, exists := mcpServers["ahd-figma"]; exists && !registerForce {
		return fmt.Errorf("already registered (use --force to overwrite)")
	}

	// Add our MCP server entry
	mcpServers["ahd-figma"] = map[string]interface{}{
		"command": binaryPath,
		"args":    []string{"mcp"},
	}
...
func registerZed(home string, editor editorConfig, binaryPath string) error {
	...
	if _, exists := ctxServers["ahd-figma"]; exists && !registerForce {
		return fmt.Errorf("already registered (use --force to overwrite)")
	}

	ctxServers["ahd-figma"] = map[string]interface{}{
		"command":  binaryPath,
		"args":     []string{"mcp"},
