...
The output is valid JSON that can be piped directly to batch:
  ai-happy-design examples carousel > /tmp/ops.json && ai-happy-design batch /tmp/ops.json`,
...
func listExampleCategories() error {
	...
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Usage: ai-happy-design examples <category>")
	fmt.Fprintln(os.Stderr, "Pipe:  ai-happy-design examples carousel > /tmp/ops.json && ai-happy-design batch /tmp/ops.json")
	return nil
}
...
func showExample(category string) error {
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "Unknown category %q. Run 'ai-happy-design examples' to list categories.\n", category)
