...
func LLMCatalog() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"workflow": map[string]interface{}{
			...
			"designSystemAnalysis": map[string]interface{}{
				"when": "Creating designs in existing Figma files that already have content.",
				"how":  "Call design_system.analyze first to discover existing styles, variables, and components. Returns categorized rules (colors, typography, spacing, effects, components) with IDs for reuse.",
				"why":  "Prevents creating duplicate styles and ensures designs match existing patterns in the file.",
			},
			"exportAndVerify": map[string]interface{}{
				"when": "After creating or editing a design, to visually confirm the result.",
				"how":  "Call export.image {nodeId, scale:2}. The image auto-saves to the OS temp directory. The response includes the file path — use your file reader tool to visually inspect the exported image.",
				"why":  "Visual verification catches layout issues, text overflow, and balance problems that JSON data alone cannot reveal.",
			},
			"compactTree": map[string]interface{}{
				"when": "Exploring document structure or discovering what's on a page without consuming excessive tokens.",
				"how":  "Call node.get_tree {nodeId, compact:true}. Returns a flat array of [{id, type, name, x, y, w, h, childCount, parentId, depth}] instead of the full nested tree.",
				"why":  "3-5x fewer tokens than the full tree. Use for structural discovery before targeted edits.",
			},
			...
			"validateBeforeBatch": map[string]interface{}{
				"builtin":   "batch and bulk.execute auto-apply all normalization — no separate validate step needed.",
				...
				"what":      "Auto-corrected: markdown fences, JSON comments (// and /* */), type→command, top-level props into params, dict wrapper unwrap.",
		...
		"playbook": []string{
			...
			"2. ANALYZE EXISTING: If the Figma file already has content, call design_system.analyze to discover existing styles, variables, and components. Reuse them instead of creating duplicates.",
			"3. COMPUTE TOKENS: Call design.compute_tokens with your canvas width and height. This returns ALL sizing values (font sizes, spacing, padding, layout type, card widths) as concrete pixel values. Use these EXACT values — do NOT calculate sizes yourself.",
			"4. FIND SPACE: Call document.find_free_space with your desired width/height to get exact x,y coordinates for new frames. NEVER guess frame positions.",
			...
			"7b. AUTO-NORMALIZED: batch and bulk.execute both auto-fix LLM output (fences, JSON comments, type→command, top-level params). Just write JSON and run batch — no separate validate step needed. Use --no-fix to disable.",
			...
			"11. EFFECTS: Shadows on cards/CTAs, gradients on hero backgrounds, blur for glass. See designDecisions.",
			"12. LAYERS: Name everything descriptively. Group in frames. Order back-to-front.",
			"13. EXPORT & VERIFY: export.image scale=2. Image auto-saves to temp dir. Read the file to visually inspect — does it look proportionate?",
			"14. EXPLORE STRUCTURE: Use node.get_tree with compact:true for token-efficient structural discovery before targeted edits.",
		},
