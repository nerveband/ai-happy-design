...
var toolDescriptions = map[string]map[string]string{
	...
	"text": {
		...
		"set_content":    "Replace text content. WARNING: assigning characters resets character-range styling. For rich text, call text.get_segments first, then reapply formatting with text.set_range_style. Params: nodeId, content",
		"get_content":    "Read text content and aggregate properties. Mixed values are returned as 'mixed'; use text.get_segments for per-range formatting. Params: nodeId",
		...
		"get_segments":   "Get styled text segments by character range. Use before replacing rich text and reapply styles with set_range_style afterward. Params: nodeId",
